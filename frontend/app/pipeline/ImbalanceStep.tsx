import type { ImbalancedMethod, ImbalancedRequest } from "~/imbalance/api";

const ImbalancedStep = function ({
  item,
  onRemove,
}: {
  item: ImbalancedRequest;
  onRemove: () => void;
}) {
  return (
        ...
        <div className="flex-1 pt-1">
          <div className="flex justify-between items-start">
            <h4 className="text-xs font-black text-white uppercase tracking-wider">
              Imbalanced: {formatMethod(item.method)}
            </h4>
...
function formatMethod(method: ImbalancedMethod) {
  return method
    .split("_")
    .map((word) => word[0].toUpperCase() + word.slice(1))
    .join(" ");
}
export default ImbalancedStep;
